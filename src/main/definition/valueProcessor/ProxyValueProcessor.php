...
final class ProxyValueProcessor implements ValueProcessor, ClassResolverAware
{
    ...
    public function process(
        ...
        $proxyName = "{$classObject->name()}\\{$shortClassName}_AutoGeneratedProxy";
        try {
            $proxyContent = $this->proxyGenerator->generate($classObject, $proxyName);
        ...
        if (!class_exists($proxyName, false)) {
            try {
                $this->classMaterializer->materialize($proxyName, $proxyContent);
        ...
        $proxyDefinition = self::createProxyDefinition($proxyName, $definition, $lifetime);
